...
from src.utils.constants import (
    MARGIN,
    THAUM_ASPECTS_INVENTORY_SLOTS_X,
...
def createButtonsAndText(
    ...
    x: int = pointTextAnchor.x,
    y: int = pointTextAnchor.y,
    movable=True,
) -> list[Text]:
    mainText = Text(
        x,
...
(Кстати, это окошко тоже можно переместить, передвинув черную точку в левом верхнем углу).""",
    ...
    movablePoint = Point(
        MARGIN + MARGIN + DEFAULT_FONT.pointSize() * 2 * 16, MARGIN + 3.4 * (DEFAULT_FONT.pointSize() * 2), movable=True
    )
...
def configureThaumWindowCoords(UI: OverlayUI):
    ...
    rectThaumWindow = UI.addObject(Rect(rectLT.x, rectLT.y, rectRB.x, rectRB.y, dashed=True, color=QColor("yellow")))
...
чтобы прямоугольник обозначал границу этого окна.""",
        confirmThaumWindowSlots,
        [UI, rectThaumWindow.LT.x, rectThaumWindow.LT.y, rectThaumWindow.RB.x, rectThaumWindow.RB.y],
        enroll,
        [UI],
    )
...
def confirmThaumWindowSlots(UI, LTx, LTy, RBx, RBy):
    ...
    def saveControls():
        saveThaumControlsConfig(
            ...
            pointPapers,
            rectAspectsListing.LT,
            rectAspectsListing.RB,
...
не передвигайте его по экрану !!)""",
    ...
    pointWritingMaterials = UI.addObject(
        Point(LTx + Ws * 1, topSlotsY, movable=True, color=QColor("yellow"))
    )  # writing materials
    ...
    UI.addObject(
        Point(rectAspectsRB.x, rectAspectsRB.y, movable=True, onMoveCallback=updateListingRectCoords)
    )  # aspects listing rectangle

    aspectsScrollY = LinkableValue(LTy + Hs * 7.65)
    pointAspectsScrollLeft = UI.addObject(
        Point(LTx + Ws * 2, aspectsScrollY, movable=True, color=QColor("lightblue"))
    )  # aspects scroll left
    pointAspectsScrollRight = UI.addObject(
        Point(LTx + Ws * 3.5, aspectsScrollY, movable=True, color=QColor("lightblue"))
    )  # aspects scroll right

    aspectsMixY = LinkableValue(LTy + Hs * 9)
    pointAspectsMixLeft = UI.addObject(
        Point(LTx + Ws * 1, aspectsMixY, movable=True, color=QColor("pink"))
    )  # aspects mix left
    pointAspectsMixCreate = UI.addObject(
        Point(LTx + Ws * 2.75, aspectsMixY, movable=True, color=QColor("pink"))
    )  # aspects mix create
    pointAspectsMixRight = UI.addObject(
        Point(LTx + Ws * 4.5, aspectsMixY, movable=True, color=QColor("pink"))
    )  # aspects mix right
    ...
    def updateHexagonsCoords():
        ...
        for i in range(-THAUM_HEXAGONS_SLOTS_COUNT // 2, THAUM_HEXAGONS_SLOTS_COUNT // 2 + 1):
            ...
            deg30HexagonsLines[idx].S.y = (
                rectHexagonsCC.y + sin(pi / 6) * rad - (i < 0) * i * slotSizeY / 2 - (i > 0) * i * slotSizeY
            )
            deg30HexagonsLines[idx].E.y = (
                rectHexagonsCC.y - sin(pi / 6) * rad - (i > 0) * i * slotSizeY / 2 - (i < 0) * i * slotSizeY
            )
    ...
    UI.addObject(Point(rectHexagonsCC.x, rectHexagonsCC.y, movable=True, onMoveCallback=updateHexagonsCoords))
    UI.addObject(
        Point(rectHexagonsCC.x, rectHexagonsTy, movable=True, onMoveCallback=updateHexagonsCoords)
    )  # hexagons rectangle
...
def chooseThaumVersion(UI: OverlayUI):
    ...
    def onSubmit():
        if selectedVersion[0] is None:
            logging.warning("Enter pressed in dialogue but thaum version is not selected")
            return
...
def beReadyForStartSolving(UI: OverlayUI, TI: ThaumInteractor):
    logging.info("Be ready for solving scenario started")
    UI.clearAll()
    UI.createExitButton()

    createNextBackButtonsAndText(
        UI,
        """Сейчас нейросеть будет определять аспекты, находящиеся на поле.
...
def detectionAspectsDialogue(UI, TI):
    TI.scrollToLeftSide()
    logging.info("TI successfully detected all aspects")

    UI.clearAll()
    exitButton = UI.createExitButton()
    (mainText, nextButton, backButton) = createNextBackButtonsAndText(
        UI,
        """Нейросеть определила аспекты в инвентаре и их количество.
...
Перелистывать страницы следует исключительно кнопками, нарисованными поверх игры""",
    ...
    def cancelAspectChanges():
        logging.info("Aspect changing canceled")
        switchToMainDialogue()

    def confirmAspectChanges():
        logging.info("Aspect changing confirmed")
        ...
        TI.setAspectIntoAvailables(newAspect, currentAspectCellCoords[0], currentAspectCellCoords[1])
        logging.info(f"All new available aspects: {TI.availableAspects}")
        switchToMainDialogue()

    def confirmAspectIsNone():
        logging.info("Aspect is none changing confirmed")
        prevAspect = TI.getAspectByCellCoords(*currentAspectCellCoords)
    ...
    [cellMainText, cellBackButton, cellNextButton, cellIsNoneButton] = createButtonsAndText(
        UI,
        """Чтобы изменить аспект в ячейке, выбери его из списка ниже
Чтобы изменить его количество, испоьзуй клавиши цифр [0-9] и [Backspace]""",
    ...
    currentAspectImage = Image(textXCoord + MARGIN * 2, textYCoord - MARGIN * 0.9, MARGIN * 2, MARGIN * 2, None)
    ...
    cellDialogueObjects += [
        ...
        cellBackButton,
        cellIsNoneButton,
        currentAspectInfoText,
        currentAspectMainText,
        currentAspectImage,
    ]
    ...
    def switchToMainDialogue():
        logging.info("Switching to a main change inventory apsects dialogue...")
        UI.setObjectsVisibility(cellDialogueObjects, False)
    ...
    def switchToCellDialogue():
        logging.info("Switching to a directly change aspect dialogue...")
        UI.removeObjects(cellsObjects)
    ...
    logging.info("UI to change detected aspects in inventory shown")


def runResearching(UI: OverlayUI, TI: ThaumInteractor):
    logging.info("Run researching scenario started")
    UI.clearAll()
    ...
    def updateCellsImage():
        ...
        logging.debug("New images placed to all cells")
    ...
    def onClickCellIsNone():
        ...
        if existingAspects[0].get(coords) is not None:
            del existingAspects[0][coords]
    ...
    def startCellDialogue(cell: Cell):
        ...
        logging.debug("Showed cell state selecting dialogue")
    ...
    def exitCellDialogue():
        ...
        selectedCell[0].object.setColor(newColor)
        selectedCell[0] = None
        logging.debug("Exit cell state selecting dialogue. Showed only cells field")
    ...
    for ix in range(-THAUM_HEXAGONS_SLOTS_COUNT // 2 + 1, THAUM_HEXAGONS_SLOTS_COUNT // 2 + 1):
        for iy in range(
            ...
            cellAspectImageObject = Image(hexagonCenterX, hexagonCenterY - imageSide, imageSide, imageSide, None)
    ...
    def startPuttingLinkMap():
        UI.setAllObjectsVisibility(False)
        onProcessText = UI.addObject(
            Text(
                MARGIN,
                MARGIN,
                """Подожди, решение выкладывается на поле...
...
Для экстренного закрытия программы нажми [Ctrl + Shift + Alt]""",
        ...
        puttingAspectsThread = threading.Thread(
            target=startPuttingAspects
        )  # run in thread to not blocking keys callbacks
    ...
    [activeStateText, activeStateNextButton, activeStateBackButton, backButton] = createButtonsAndText(
        UI,
        """Нейросеть определила аспекты на поле.
...
Чтобы приостановить программу, нажми [Ctrl + Shift + Пробел]""",
    ...
    multyResearchesObjects = createButtonsAndText(
        UI,
        """Начать безостановочное исследование нескольких записок.
...
В столе исследований записки быть не должно""",
    ...
    onPausedText = UI.addObject(
        Text(
            MARGIN,
            MARGIN,
            """Программа проистановлена.
